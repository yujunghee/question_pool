						...
						<div id="blist">
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0" summary="관리자 관리목록입니다.">
								...
								<tbody>
			                           ...
			                           <tr>
			                            	<td scope="col" class="first"><input type="checkbox" name="RowCheck" value="${list.user_no }" /></td>			                            	
			                                
			                                <td>${list.user_no }</td>
			                                <td class="txt_l">
			                                    <a href="view.do?user_no=${list.user_no }">${list.user_email }</a>
			                                </td>
			                                <td>${list.user_name }</td>
			                                <td>${list.user_tel }</td>			       
			                                <td class="date"><fmt:formatDate value="${list.user_regdate }" pattern="yyyy-MM-dd"/></td>			                                                              
			                           </tr>
			                            </c:forEach>
							...
							<form name="searchForm" id="searchForm" action="index.do"  method="get" >
								<div class="search">
									<select name="searchType" title="검색을 선택해주세요">
										...
										<option value="user_email" <c:if test="${param.searchType == 'user_email'}">selected</c:if>>아이디</option>
										<option value="user_name" <c:if test="${param.searchType == 'user_name'}">selected</c:if>>이름</option>
									</select>
